...
This folder creates the AWS foundation for the prod environment of the Astronomy Shop platform.
...
EKS node instance type: m7i-flex.large
NAT behavior: Enabled by example default with single NAT behavior for cost and EIP quota control.
RDS: Enabled by example default using db.t3.micro.
Redis: Enabled by example default using cache.t3.micro.

Prod is your restricted-account production profile. It demonstrates production architecture patterns, but it is still limited by one AWS account, one region, small database/cache choices, and disabled paid security services when the account rejects them.
...
The service list is used to create ECR repositories for the application microservices. It must match the prod GitOps values and ArgoCD ApplicationSet service list.
