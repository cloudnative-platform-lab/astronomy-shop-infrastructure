...
variable "eks_endpoint_public_access_cidrs" {
  type        = list(string)
  description = "Trusted public CIDRs allowed to reach the EKS API. Never use 0.0.0.0/0 in production."

  validation {
    condition     = !contains(var.eks_endpoint_public_access_cidrs, "0.0.0.0/0")
    error_message = "Production EKS API access must be restricted to trusted CIDRs."
  }
}
