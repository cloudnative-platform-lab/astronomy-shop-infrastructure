...
This folder creates the AWS foundation for the dev environment of the Astronomy Shop platform.
...
NAT behavior: Disabled by example default. Dev can use public subnets when NAT is disabled to reduce cost.
RDS: Disabled by example default.
Redis: Disabled by example default.

Dev is intentionally the smallest environment. It exists for learning, early validation, and low-cost Terraform testing. It is not meant to prove production traffic capacity.
...
The service list is used to create ECR repositories for the application microservices. It must match the dev GitOps values and ArgoCD ApplicationSet service list.
